...
class LSA():
  ...
  def buildIndex(self, docs, docIDs, k, ngram):
    """
		...
		None
		"""
    print("Using LSA model using {} concepts to evaluate relevance ...".format(k))
    x = 1
    y = 2
    if ngram == 1:
      x = 1
    ...
    joined_docs = [' '.join(sentence) for sentence in joined_docs]

    tfidf_vectorizer = TfidfVectorizer(ngram_range=(x,y))
    tdidf_matrix = tfidf_vectorizer.fit_transform(joined_docs)
   
    svd = TruncatedSVD(n_components= k, random_state=42)
    reduced_matrix = svd.fit_transform(tdidf_matrix)
