...
class SearchEngine:
  ...
  def __init__(self, args):
    ...
    elif self.args.method == "glsa" :
      self.informationRetriever = Glasgow_LSA()
    elif self.args.method == "vsm":
      self.informationRetriever = Hybrid()
    elif self.args.method == "lda":
      self.informationRetriever = LDA()
    elif self.args.method == "baseline":
      self.informationRetriever = Baseline()
    elif self.args.method == "bm25":
      self.informationRetriever = BM25()
  ...
  def preprocessQueries(self, queries):
    ...
    json.dump(stopwordRemovedQueries,
  ...
  def preprocessDocs(self, docs):
    ...
    json.dump(reducedDocs, open(self.args.out_folder + "reduced_docs.txt",
                                'w'))
  ...
  def evaluateDataset(self):
    ...
    self.informationRetriever.buildIndex(processedDocs, doc_ids, self.k, self.ngram)
    ...
    for k in range(1, 11):
      precision = self.evaluator.meanPrecision(doc_IDs_ordered, query_ids,
                                               qrels, k)
      ...
      MAP = self.evaluator.meanAveragePrecision(doc_IDs_ordered, query_ids,
                                                qrels, k)
      MAPs.append(MAP)
    ...
    plt.xlabel("k")
    plt.savefig("final_plots/"+self.args.method + "_" + self.args.ngram + "_" + str(self.k) + ".png")
  ...
  def handleCustomQuery(self):
    ...
    #spell correction for custom query
    blob = TextBlob(query)
    query = blob.correct()

    processedQuery = self.preprocessQueries([str(query)])[0]
...
if __name__ == "__main__":
  ...
  parser.add_argument('-method',
                      default="lsa",
                      help="lsa|vsm|glsa|lda|baseline|ir")
  ...
  parser.add_argument('-k',
                      default= "250",
                      help="no of concepts to be used by lsa")
